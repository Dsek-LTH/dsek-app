...
const formatShortDate = (date: Date, monthsList?: string[]): string => {
  return monthsList
    ? `${padLeft(date.getDate())} ${getMonth(date, monthsList)}`
    : `${padLeft(date.getDate())} / ${padLeft(date.getMonth() + 1)} `;
};
...
const weekdaysLong = ['Söndag', 'Måndag', 'Tisdag', 'Onsdag', 'Torsdag', 'Fredag', 'Lördag'];
...
const formatReadableDateTime = (date: Date, dateTimeStrings: DateTimeStrings): string => {
  ...
  if (timeSince < 1209600000 || date.getMonth() === new Date().getMonth()) {
    return `${getWeekday(date, dateTimeStrings.weekdaysLong)} ${formatShortDate(
      date,
      dateTimeStrings.monthsLong
    )}, ${formatTime(date)} `;
  }
  // If this year
...
const DateTime = {
  ...
  getWeekday,
  formatWithWeekday,
  formatReadableDateTime: (date: Date) =>
    formatReadableDateTime(date, { weekdaysLong, weekdaysShort, monthsLong, monthsShort }),
};
